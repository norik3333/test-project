---
description: 双雅 Water Spider全自動モード起動
---

# 双雅 Water Spider全自動モード
...
```bash
souga auto
souga auto --max-duration 60 --interval 30
souga auto --dry-run   # テスト実行
```

**MCPツール経由（miyabi-integration MCP が有効な場合のみ）**:

`souga__auto` ツールを使用。MCP サーバーは `.claude/mcp-servers/miyabi-integration.js`。

> ⚠️ `souga__auto` MCP ツールが使えない場合はCLI方式を使うこと。
> `souga-integration.js` は存在しないため `souga` MCPサーバーは実態は `miyabi-integration.js` を指す。
...
# 最大Issue数と間隔を指定
npx souga auto --max-issues 10 --interval 30

# 1回だけ実行（ポーリングなし）
npx souga auto --max-issues 5 --interval 0
```
...
```bash
# maxIssues到達で自動停止
npx souga auto --max-issues 3

# interval=0 で1回のみ実行
npx souga auto --interval 0
```
...
Claude: [souga__auto({ maxIssues: 5 }) を実行]
...
Claude: [souga__auto({ maxIssues: 10, interval: 30 }) を実行]
...
**通常運用**:
```bash
npx souga auto --max-issues 10 --interval 60
```

**夜間バッチ処理**:
```bash
npx souga auto --max-issues 50 --interval 0
```

**テスト・検証**:
```bash
npx souga auto --max-issues 1 --interval 0 --dry-run
```
