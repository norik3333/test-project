---
description: 新しいSougaプロジェクト作成
---

# Sougaプロジェクト作成

新しいSougaプロジェクトを作成します。GitHub連携、Agent設定、Claude Code統合を含む完全なセットアップを実行します。
...
```
パブリックプロジェクト作成:
souga__init({ projectName: "my-awesome-app" })

プライベートプロジェクト作成:
souga__init({ projectName: "my-secret-project", private: true })

npm installスキップ:
souga__init({ projectName: "quick-setup", skipInstall: true })
```

## 自動セットアップ内容

`souga__init` を実行すると、以下が自動的に実行されます:
...
├── mcp.json                # MCPサーバー設定
├── commands/
│   ├── souga-agent.md     # /souga-agent
│   ├── souga-status.md    # /souga-status
│   └── souga-init.md      # /souga-init
├── hooks/
│   └── format.sh           # 自動フォーマット
...
```bash
# パブリックプロジェクト作成
npx souga init my-awesome-app

# プライベートプロジェクト作成
npx souga init my-secret-project --private

# npm installスキップ
npx souga init quick-setup --skip-install
```
...
次のステップ:
  ...
  npx souga status    # ステータス確認
  npx souga auto      # 全自動モード起動
```
