...
```
TODOをスキャンして表示:
souga__todos({ path: "./src" })

TODOを検出して自動的にIssue作成:
souga__todos({ path: "./src", autoCreate: true })
...
```bash
# TODOをスキャンして表示
npx souga todos --path ./src

# 自動的にIssue作成
npx souga todos --path ./src --auto-create

# 特定のパターンのみ検出
npx souga todos --path . --pattern "TODO|FIXME"

# ドライラン（Issue作成しない）
npx souga todos --path ./src --auto-create --dry-run
```
...
推奨アクション:
  1. npx souga todos --auto-create でIssue化
  2. 高優先度(P0/P1)から対応
```
...
🤖 Generated by Souga Todos Auto Mode
...
Claude: [souga__todos({ path: "./src" }) を実行]
...
Claude: [souga__todos({ path: ".", autoCreate: true }) を実行]
...
- **Label付与**: 自動付与されるLabelは `.souga.yml` でカスタマイズ可能
